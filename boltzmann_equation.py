...
def main(args: argparse.ArgumentParser):
    ...
    # Grids
    q_arr = np.linspace(q_min, q_max, args.N_q, dtype=np.float64)
    p_arr = np.linspace(p_min, p_max, args.N_p, dtype=np.float64)
    t_arr = np.linspace(t_min, t_max, args.N_t, dtype=np.float64)
    ...
    #h_exact = np.array([[p for q in q_arr] for p in p_arr])
    #print(pv[:, :, 50]==h_exact)
